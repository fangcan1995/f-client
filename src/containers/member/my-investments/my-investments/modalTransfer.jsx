...
import {Loading,NoRecord,BbhAlert,Posting} from '../../../../components/bbhAlert/bbhAlert';
...
class ModalTransfer extends React.Component {
    ...
    checkAmount(pram){
        //1 验证输入是否正确
        let {proMinInvestAmount,transFinanced,proIncreaseAmount}=this.props.memberInvestments.myInvestments.transferInfo;
        let result=checkMoney({
            'value':pram,
            'type':0,
            'min_v':parseInt(proMinInvestAmount),//
            'max_v':parseInt(transFinanced), //
            'label':'转让金额',
            'interval':parseInt(proIncreaseAmount)  //
        });
        return result;
    }
    handleSubmit(){
                ...
                {postResult == `` ?
                    <div className="form__wrapper">
                        <dl className="form__bar">
                            ...
                            <dd><i id="Accountbalance" className="money">{addCommas(transferInfo.transFinanced)}</i> 元
                            </dd>
                        </dl>
                        <dl className="form__bar">
                        ...
                        <dl className="form__bar">
                            <p>
                                <Checkbox onChange={this.onChange}>我已阅读并同意
                                    <a href="/transfer.html" target="_blank">《巴巴汇债权转让服务协议》</a>
                                </Checkbox>
                            </p>
                        </dl>
                        ...
                        <div className="form__bar">
                            {isPosting ?
                                <Button className="button unable" style={{marginTop: '30px'}}>
                                    <Posting isShow={isPosting}/></Button>
                                :
