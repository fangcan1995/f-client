...
export default class InvestList extends React.Component{
    ...
    //数据设置
    /*dataSet={
        "url":`http://172.16.1.221:9090`,
        "pageNum":1,
        "pageSize":10,
        "filter":{
            greenHand:'',  //是否新手 0全部 2新手 1普通
            loanApplyExpiry:'',
            rateBegin:'',
            rateEnd:'',
        }
    ...
    loadData(currentPage,pageSize,filter){
        ...
        /*let conditions = "";
        for(var item in filter){
            if(filter[item]&&filter[item]!=''){
                conditions += "&"+item+"="+filter[item];
            }
        }
        console.log(`${this.dataSet.url}&pageNum=${currentPage}&pageSize=${pageSize}${conditions}`);
        const _this = this;
        jQuery.ajax({
